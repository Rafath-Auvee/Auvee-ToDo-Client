...
const RequireAuth = ({ children }) => {
  ...
  if (user.providerData[0]?.providerId === "password" && !user.emailVerified) {
    return (
      <div class="flex flex-col items-center justify-center w-screen h-screen bg-indigo-600">
        <h3 className="text-2xl text-white md:text-7xl lg:text-4xl">
          Email is not VERIFIED!!
        </h3>
        <h5 className="text-2xl text-white md:text-7xl lg:text-4xl">
          {" "}
